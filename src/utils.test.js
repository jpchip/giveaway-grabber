...
describe('checkStringForWords', () => {
	...
	test('does not match partial words in string', () => {
		const match = checkStringForWords(
			'cat',
			'That was a categorically good vacation!'
		);
	...
	test('matches words with spaces in them', () => {
		let match = checkStringForWords(
			'kindle edition,something',
			'I bought the Kindle Edition for xmas'
		);
		expect(match).toEqual('kindle edition');
		match = checkStringForWords(
			'dog,bought the',
			'I bought the Kindle Edition for xmas'
		);
